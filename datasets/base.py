...
class BaseDataset(Dataset):
    ...
    def get_pix(self, idx):
        if self.split.startswith('train'):
            ...
            sample = {'img_idxs': img_idxs, 'pix_idxs': pix_idxs, 'haz_rgb': self.haz_images[img_idxs, pix_idxs][:, :3],
                      "clear_rgb": self.clear_images[img_idxs, pix_idxs][:, :3], "pose": self.poses[img_idxs],
                      "direction": self.directions[pix_idxs],
                      "atmospheric_lights": self.atmospheric_lights[img_idxs],
                      "atmospheric_lights_mean": self.atmospheric_lights.mean()}
        else:
            # idx = 80
