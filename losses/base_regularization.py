...
class AtmosphericLightsLoss(torch.autograd.Function):
    @staticmethod
    def forward(ctx, p_rgbs, atmospheric_lights, rays_a, vr_samples):
        loss = vren.atmospheric_lights_fw(p_rgbs, atmospheric_lights, rays_a, vr_samples)
        ctx.save_for_backward(p_rgbs, atmospheric_lights, rays_a, vr_samples)
        return loss

    @staticmethod
    def backward(ctx, dL_dloss):
        p_rgbs, atmospheric_lights, rays_a, vr_samples = ctx.saved_tensors
        dL_dp_rgbs = vren.atmospheric_lights_bw(dL_dloss, p_rgbs, atmospheric_lights, rays_a, vr_samples)
        return dL_dp_rgbs, None, None, None
