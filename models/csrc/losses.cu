...
template <typename scalar_t>
__global__ void atmospheric_lights_fw_kernel(
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> p_rgbs,
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> atmospheric_lights,
    const torch::PackedTensorAccessor64<int64_t, 2, torch::RestrictPtrTraits> rays_a,
    const torch::PackedTensorAccessor64<int64_t, 1, torch::RestrictPtrTraits> vr_samples,
    torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> loss
){
    const int n = blockIdx.x * blockDim.x + threadIdx.x;
    if (n >= rays_a.size(0)) return;
    ...
    for(int i=0;i<N_samples;i++){
        const scalar_t s = start_idx+i;
        loss[ray_idx][0] += powf( ( p_rgbs[s][0] - atmospheric_lights[ray_idx][0] ), 2 ) / N_samples;
        loss[ray_idx][1] += powf( ( p_rgbs[s][1] - atmospheric_lights[ray_idx][1] ), 2 ) / N_samples;
        loss[ray_idx][2] += powf( ( p_rgbs[s][2] - atmospheric_lights[ray_idx][2] ), 2 ) / N_samples;
    }

}

torch::Tensor atmospheric_lights_fw_cu(
    const torch::Tensor p_rgbs,
    const torch::Tensor atmospheric_lights,
    const torch::Tensor rays_a,
    const torch::Tensor vr_samples
){
    const int N_rays = rays_a.size(0), N = p_rgbs.size(0);
    const int threads = 256, blocks = (N_rays+threads-1)/threads;

    auto loss = torch::zeros({N_rays,3},p_rgbs.options());

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(p_rgbs.type(), "atmospheric_lights_fw_cu",
    ([&] {
        atmospheric_lights_fw_kernel<scalar_t><<<blocks, threads>>>(
            p_rgbs.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            atmospheric_lights.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            rays_a.packed_accessor64<int64_t, 2, torch::RestrictPtrTraits>(),
            vr_samples.packed_accessor64<int64_t, 1, torch::RestrictPtrTraits>(),
...
template <typename scalar_t>
__global__ void atmospheric_lights_bw_kernel(
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> dL_dloss,
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> p_rgbs,
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> atmospheric_lights,
    const torch::PackedTensorAccessor64<int64_t, 2, torch::RestrictPtrTraits> rays_a,
    const torch::PackedTensorAccessor64<int64_t, 1, torch::RestrictPtrTraits> vr_samples,
    ...
    for(int i=0;i<N_samples;i++){
        const scalar_t s = start_idx+i;
        dL_dp_rgbs[s][0] = dL_dloss[ray_idx][0] * 2 * ( p_rgbs[s][0] - atmospheric_lights[ray_idx][0] ) / N_samples;
        dL_dp_rgbs[s][1] = dL_dloss[ray_idx][1] * 2 * ( p_rgbs[s][1] - atmospheric_lights[ray_idx][1] ) / N_samples;
        dL_dp_rgbs[s][2] = dL_dloss[ray_idx][2] * 2 * ( p_rgbs[s][2] - atmospheric_lights[ray_idx][2] ) / N_samples;
    }
}


torch::Tensor atmospheric_lights_bw_cu(
    const torch::Tensor dL_dloss,
    const torch::Tensor p_rgbs,
    const torch::Tensor atmospheric_lights,
    const torch::Tensor rays_a,
    ...
    auto dL_dp_rgbs = torch::zeros({N,3}, dL_dloss.options());

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(p_rgbs.type(), "atmospheric_lights_bw_cu",
    ([&] {
        atmospheric_lights_bw_kernel<scalar_t><<<blocks, threads>>>(
            dL_dloss.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            p_rgbs.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            atmospheric_lights.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            rays_a.packed_accessor64<int64_t, 2, torch::RestrictPtrTraits>(),
            vr_samples.packed_accessor64<int64_t, 1, torch::RestrictPtrTraits>(),
