...
torch::Tensor atmospheric_lights_fw(
    const torch::Tensor p_rgbs,
    ...
    CHECK_INPUT(p_rgbs);
    CHECK_INPUT(atmospheric_lights);
    CHECK_INPUT(rays_a);
    CHECK_INPUT(vr_samples);

    return atmospheric_lights_fw_cu(p_rgbs,atmospheric_lights,rays_a,vr_samples);
}


torch::Tensor atmospheric_lights_bw(
    const torch::Tensor dL_dloss,
    const torch::Tensor p_rgbs,
    const torch::Tensor atmospheric_lights,
    const torch::Tensor rays_a,
    ...
    return atmospheric_lights_bw_cu(dL_dloss,p_rgbs,atmospheric_lights,rays_a,vr_samples);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("atmospheric_lights_fw", &atmospheric_lights_fw);
    m.def("atmospheric_lights_bw", &atmospheric_lights_bw);
